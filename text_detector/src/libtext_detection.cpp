...
void TextDetector::setCannyParams(const int& lower, const int& upper, const int& size) {
    canny_param_lower_ = lower;
    canny_param_upper_ = upper;
    canny_kernel_size_ = size;
}

void TextDetector::thresholdImage(cv::Mat& img) {
    if (img.empty()) {
        return;
    }
...
void TextDetector::findGoodContours() {
...
void TextDetector::fitRect(cv::Mat& board) {
    int area_curr = 0;
    int area_prev = 0;
    center_.x = -1;
    center_.y = -1;
...
void TextDetector::drawContours(cv::Mat& board) {
    cv::drawContours(board, good_contours_, -1, cv::Scalar(255, 255, 255), 2);
}

std::pair<int, int> TextDetector::getCentre() {
    return centre_;
}

cv::Mat TextDetector::getThresh() {
    return thresh_img_;
}

cv::Point2f* TextDetector::getCorners() {
    return rect_points;
}

double TextDetector::getDistance() {
    return distance_;
}

double TextDetector::getArea() {
    return area_;
}

void TextDetector::setHSVMin(const int& h, const int& s, const int& v) {
    hsv_min_ = cv::Scalar(h, s, v);
}

void TextDetector::setHSVMax(const int& h, const int& s, const int& v) {
    hsv_max_ = cv::Scalar(h, s, v);
}

void TextDetector::setMinArea(const int& area) {
