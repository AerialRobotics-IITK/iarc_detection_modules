...
cv::Mat TextDetect::findWhiteTextBox(cv::Mat &frame) {
  ...
  std::vector<cv::Vec4i> hierarchy;
  cv::Mat WhiteTextBox, drawing = cv::Mat::zeros(frame.size(), CV_8UC3);
  cv::findContours(processed_frame_, list_contours, hierarchy, CV_RETR_EXTERNAL,
  ...
  for (int i = 0; i < list_contours.size(); i++) {
    ...
    detector_->detectAndCompute(text_resize, cv::noArray(), keypoints2,
                                descriptors2);
...
>>>>>>> 54a3e08efbb4aaba47096542e3e2b57243062432
    ...
    cv::drawMatches(src_, keypoints1_, text_resize, keypoints2, good_matches,
                    img_matches_, cv::Scalar::all(-1), cv::Scalar::all(-1),
                    std::vector<char>());
