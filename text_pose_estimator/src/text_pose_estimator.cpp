...
void TextPoseEstimatorNode::init(ros::NodeHandle& nh, ros::NodeHandle& nh_private) {
    centre_coord_sub_ = nh.subscribe("centre_coord", 10, &TextPoseEstimatorNode::centreCallback, this);
    odom_sub_ = nh.subscribe("odom", 10, &TextPoseEstimatorNode::odomCallback, this);
    corners_sub_ = nh.subscribe("corners", 10, &TextPoseEstimatorNode::cornersCallback, this);
...
void TextPoseEstimatorNode::run() {
...
Eigen::Vector3d TextPoseEstimatorNode::calculateGlobCoord(const double& img_x, const double& img_y, const double& dist) {
...
Eigen::Vector3d TextPoseEstimatorNode::calculateQuadCoord(const double& img_x, const double& img_y, const double& dist) {
    pose_est_.getDistance(dist_);
    pose_est_.setImgVec(img_x, img_y);
    pose_est_.CamToQuad();
...
Eigen::Vector3d TextPoseEstimatorNode::calculateQuadCoordForDist(const double& img_x, const double& img_y) {
    pose_est_.setImgVec(img_x, img_y);
    pose_est_.CamToQuadForDist();
    return pose_est_.getQuadCoord();
}

Eigen::Vector3d TextPoseEstimatorNode::calculatePlateFrontVec() {
...
void TextPoseEstimatorNode::calculateScalingFactor() {
...
void TextPoseEstimatorNode::publishCorrectionAngles() {
...
void TextPoseEstimatorNode::centreCallback(const detector_msgs::Centre& msg) {
    centre_coord_ = msg;
}

void TextPoseEstimatorNode::odomCallback(const nav_msgs::Odometry& msg) {
    odom_ = msg;
}

void TextPoseEstimatorNode::cornersCallback(const detector_msgs::Corners& msg) {
    corners_ = msg;
}
