...
void PlateDetector::findGoodContours() {
  ...
  cv::Canny(thresh_img_, canny_img, canny_param_low_, canny_param_upper_,
            canny_kernel_size_);
  ...
  cv::findContours(thresh_img_, contours, hierarchy, CV_RETR_TREE,
...
void PlateDetector::fitRect(cv::Mat &board) {
  ...
  distance_ = sqrt((rect_points[1].x - rect_points[0].x) *
