...
void DockDetectorNode::init(ros::NodeHandle& nh)
{
  ...
  nh_private.getParam("dock_radius", dock_radius_);
  nh_private.getParam("inter_centre_dist", inter_centre_dist);
  ...
  detect_.setInterCentreDist(inter_centre_dist);
...
void DockDetectorNode::run()
{
  ...
  detect_.findAllContours(img_);
  detect_.circleDet(img_);  // Comment this line while implementing like that of helipad_det.
