...
void PlateDetectorROS::init(ros::NodeHandle &nh) {
  ...
  img_sub_ = nh.subscribe("/firefly/camera_front/image_raw", 1,
                          &PlateDetectorROS::imageCallback, this);
...
void PlateDetectorROS::run() {
  ...
  sensor_msgs::ImagePtr thresh_msg =
      cv_bridge::CvImage(std_msgs::Header(), "mono8", detect_.getThresh())
          .toImageMsg();
  sensor_msgs::ImagePtr contour_msg =
      cv_bridge::CvImage(std_msgs::Header(), "bgr8", img_).toImageMsg();
