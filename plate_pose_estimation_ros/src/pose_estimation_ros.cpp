...
void PlatePoseEstimationROS::init(ros::NodeHandle &nh) {
  centre_coord_sub_ =
      nh.subscribe("/firefly/plate_detector_node/centre_coord", 10,
  ...
  glob_coord_pub_ =
      nh_private.advertise<util_msgs::global_coord>("estimated_coord", 10);
...
void PlatePoseEstimationROS::centreCallback(const util_msgs::centre &msg) {
  centre_coord_ = msg;
}

void PlatePoseEstimationROS::odomCallback(const nav_msgs::Odometry &msg) {
  odom_ = msg;
}
